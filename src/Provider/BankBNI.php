 ...
 */
class BankBNI extends AbstractProvider
{
    /**
     * @var string
     */
     ...
     * @var string
     */
    protected $providerName = 'Bank BNI Special Rate';

    /**
     * @var string
     */
    protected $providerUrl = 'https://www.bni.co.id/';

    /**
     * @var string
     */
    protected $ratesUrl = 'https://www.bni.co.id/id-id/beranda/informasivalas';
    ...
    public function getRates(): Rates
    {
        ...
        try {
            ...
            $tbody = $body->filter('.content-infoView table.table_info_counter');
            if (!$tbody->count()) {
                unset($tbody);
                return $this->rates;
            }
            $tbody = $tbody->eq(0)->filter('tbody tr');
            if (!$tbody->count()) {
                unset($tbody);
                return $this->rates;
            }
            $tbody->each(function (HtmlPageCrawler $crawler) {
                ...
                $buy = str_replace('.', '', trim($td->eq(2)->text()));
                $sell = str_replace('.', '', trim($td->eq(1)->text()));
