 ...
 */
class BankBCA extends AbstractProvider
{
    /**
     * @var string
     */
    protected $providerCode = 'BCA';

    /**
     * @var string
     */
    protected $providerName = 'Bank Central Asia E-Rate';

    /**
     * @var string
     */
    protected $providerUrl = 'https://www.bca.co.id';

    /**
     * @var string
     */
    protected $ratesUrl = 'https://www.bca.co.id/en/e-rate';
    ...
    public function getRates(): Rates
    {
        ...
        $client = new Client([
            RequestOptions::HEADERS => [
                ...
                'referer' => 'https://www.bca.co.id/en',
        ...
        try {
            ...
            $tbody = $body->filter('.kurs-e-rate table.table tbody tr');
            if (!$tbody->count()) {
                unset($tbody);
                return $this->rates;
            }
            $tbody->each(function (HtmlPageCrawler $crawler) {
                ...
                $buy = abs(str_replace(',', '', trim($td->eq(1)->text())));
                $sell = abs(str_replace(',', '', trim($td->eq(2)->text())));
